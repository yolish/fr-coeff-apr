    ...
    if args.mode == 'train':
        ...
        for epoch in range(n_epochs):
            ...
            for batch_idx, minibatch in enumerate(dataloader):
                ...
                if batch_idx % n_freq_print == 0:
                    if model.smooth_regression:
                        det_est_pose = apply_rev_ft(est_pose.detach())
                    else:
                        det_est_pose = est_pose.detach()
                    posit_err, orient_err = utils.pose_err(det_est_pose, gt_pose.detach())
        ...
        with torch.no_grad():
            for i, minibatch in enumerate(dataloader, 0):
                ...
                if model.smooth_regression:
                    est_pose = ft.apply_rev_ft(est_pose)
                toc = time.time()
